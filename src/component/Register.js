...
function Register(props) {
    ...
    const register= (event)=> {
        ...
        if (isFnameValid && isLnameValid && isEmailValid && isPasswordValid  && isCPasswordValid && isGenderValid &&isDateValid) {
        ...
        const validateConfirmPassword = (cpassword,password) => {
            if ((cpassword===userDetails.password ) && cpassword !="") {
               ...
               setcpasswordError('')
                ...
                setcpasswordError('Password and confirm password must be match')
                return false
            }
        }
    ...
    return (
     ...
     {!isLnameValid ? <span style={{color:'red', fontSize:'12px',position:"relative",right:"30px"}}>{lnameError}</span> : null}
      ...
      <div>
     <input type="Password" placeholder='New Password'  name="password"  onChange={(event)=>{handleChange(event)}} value={userDetails.password} /><br></br>
     {!isPasswordValid ? <span style={{color:'red', fontSize:'11px',position:"relative",right:"30px"}}>{passwordError}</span> : null}
      </div> 

      <div>
     <input type="password" placeholder='Confirm Password' name="cpassword" onChange={(event)=>{handleChange(event)}} value={userDetails.cpassword} /><br></br>
     ...
     {!isDateValid ? <span style={{color:'red', fontSize:'12px',position:"relative",right:"30px"}}>{dateError}</span> : null}
      ...
      <input  type="radio" name="gender" value="Female" style={{marginLeft:"20px"}} onClick={(event) => {handleChange(event)}} />Female
